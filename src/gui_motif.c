...
# if (XmVersion >= 1002)
		XtSetArg(al[ac], XmNtearOffModel, tearoff_val);
		ac++;
# endif
...
# endif
	...
	n++;
	XtSetArg(args[n], XmNbackPageSize, XmNONE);
	n++;
	XtSetArg(args[n], XmNbackPageNumber, 0);
	n++;
	XtSetArg(args[n], XmNbackPagePlacement, XmTOP_RIGHT);
	n++;
	XtSetArg(args[n], XmNmajorTabSpacing, 0);
	n++;
	...
	n = 0;
	XtSetArg(args[n], XmNuserData, TABLINE_MENU_CLOSE);
	n++;
	...
	n = 0;
	XtSetArg(args[n], XmNuserData, TABLINE_MENU_NEW);
	n++;
	...
	n = 0;
	XtSetArg(args[n], XmNuserData, TABLINE_MENU_OPEN);
	n++;
...
# ifdef FEAT_TOOLBAR
	if (menu_is_toolbar(parent->name)) {
		...
		if (menu_is_separator(menu->name)) {
			...
			n++;
			XtSetArg(args[n], XmNminWidth, wid);
			n++;
			XtSetArg(args[n], XmNorientation, XmVERTICAL);
			n++;
		...
		XtSetArg(args[n], XmNpositionIndex, idx);
		n++;
		if (menu->id == NULL) {
...
int		flag;
{
	...
	if (sb->id != (Widget)0) {
		...
		if (flag) {
			switch (sb->type) {
			case SBAR_LEFT:
				XtSetArg(args[n], XmNleftOffset, gui.scrollbar_width);
				n++;
				break;

			case SBAR_RIGHT:
				XtSetArg(args[n], XmNrightOffset, gui.scrollbar_width);
				n++;
				break;

			case SBAR_BOTTOM:
				XtSetArg(args[n], XmNbottomOffset, gui.scrollbar_height);
				n++;
				break;
			}
...
int		orient;	/* SBAR_VERT or SBAR_HORIZ */
{
	...
	XtSetArg(args[n], XmNorientation,
			 (orient == SBAR_VERT) ? XmVERTICAL : XmHORIZONTAL);
	n++;
	...
	case SBAR_LEFT:
		...
		n++;
		XtSetArg(args[n], XmNbottomAttachment, XmATTACH_OPPOSITE_FORM);
		n++;
		XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
		n++;
		break;
...
int		ex_cmd UNUSED;
{
	...
	XtSetArg(args[n], XmNbottomAttachment, XmATTACH_FORM);
	n++;
	XtSetArg(args[n], XmNbottomOffset, 8);
	n++;
	XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
	n++;
...
gui_mch_update_tabline(void)
{
	...
	for (tp = first_tabpage; tp != NULL; tp = tp->tp_next, ++nr) {
		...
		if (page_status == XmPAGE_INVALID
			...
			/* Add the tab */
			n = 0;
			XtSetArg(args[n], XmNnotebookChildType, XmMAJOR_TAB);
			n++;
			XtSetArg(args[n], XmNtraversalOn, False);
			n++;
...
int		do_replace;
{
	...
	n++;
	XtSetArg(args[n], XmNdialogTitle, str);
	n++;
	...
	n++;
	XtSetArg(args[n], XmNrightWidget, button_form);
	n++;
